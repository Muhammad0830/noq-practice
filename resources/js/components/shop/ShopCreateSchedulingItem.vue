  ...
  <v-expansion-panels variant="accordion">
    ...
    >
      <v-expansion-panel-title
        ...
        :collapse-icon="ChevronUp"
        color="secondary"
        class="border-0! shadow-none!"
        :class="{ 'opacity-80': !dayOfWeekItem.isOpen }"
      >
        <template #default="{ }">
          <v-row density="comfortable">
            <v-col
              class="d-flex flex-col justify-start"
              :cols="xs ? 12 : 6"
            >
              ...
              <div class="flex items-center gap-2 text-sm">
                ...
                <div class="text-primary">
                  {{ dayOfWeekItem.closed.length }} breaks
                </div>
              </div>
            </v-col>
      ...
      <v-expansion-panel-text class="bg-secondary/90">
        ...
        >
          ...
          >
            <p
              :class="{ 'cursor-default': !dayOfWeekItem.isOpen }"
              class="flex-1"
            >
              Open Hours
            </p>
            <div class="flex items-center gap-2 sm:justify-end">
              ...
              <v-text-field
                v-model="dayOfWeekItem.open.start_time"
                :disabled="!dayOfWeekItem.isOpen"
                type="time"
              ...
              <v-text-field
                v-model="dayOfWeekItem.open.end_time"
                :disabled="!dayOfWeekItem.isOpen"
                type="time"
                class="max-w-28 translate-y-2"
              />
          ...
          <li
            v-for="(item) in dayOfWeekItem.closed"
            :key="item.id"
            class="flex max-sm:flex-col items-center left-0! right-full! justify-between text-foreground"
            :class="{ 'opacity-50': !dayOfWeekItem.isOpen }"
          >
          ...
          <div
            key="add-toggle-buttons"
            class="flex items-center gap-4 justify-center sm:justify-between"
          >
            <div class="flex transition-all sm:flex-1 duration-500">
              <v-btn
                ...
                @click="addNewBreak(day as DayOfWeekItemType)"
              >
                Add Breaks
              </v-btn>
            </div>
            ...
            <div class="flex items-center gap-2">
              <span v-show="dayOfWeekItem.isOpen" />
              <span v-show="!dayOfWeekItem.isOpen" />
              <v-switch
                v-model="dayOfWeekItem.isOpen"
                inset="material"
                :true-icon="Check"
                :false-icon="X"
                class="translate-y-3"
